...
const styles = `
  ...
  .hrja-table-head { 
    display: grid; 
    grid-template-columns: 50px 1fr 1fr 100px 70px 70px 80px 80px 70px 140px; 
    gap: 8px; align-items: center; 
    background: #eef2ff; border: 1px solid #e0e7ff; border-radius: 12px 12px 0 0; 
  ...
  .hrja-row { 
    ...
    border-bottom: 1px solid #f0f0ff; transition: background .15s; font-size: 13px; 
  }
  ...
  .hrja-col-center { display: flex; align-items: center; justify-content: center; text-align: center; }
  ...
  .hrja-score { display: flex; align-items: center; gap: 4px; font-weight: 700; justify-content: center; }
  ...
  .hrja-score--processing { color: #f59e0b; font-style: italic; }
  .hrja-score--failed { color: #ef4444; }
  .hrja-cgpa { display: flex; align-items: center; gap: 4px; color: #6b7280; justify-content: center; }
  ...
  .hrja-modal { background: #fff; border: 1px solid #e0e7ff; border-radius: 20px; box-shadow: 0 24px 72px rgba(99,102,241,.2); max-width: 560px; width: 100%; padding: 28px; position: relative; animation: hrja-modal-in .25s ease; max-height: 80vh; overflow-y: auto; }
  ...
  .hrja-modal-row { margin-bottom: 14px; font-size: 14px; color: #4b5563; line-height: 1.6; }
  .hrja-modal-row strong { color: #1e1b3a; display: block; margin-bottom: 4px; }
  .hrja-modal-score { font-size: 32px; font-weight: 800; color: #7c3aed; text-align: center; margin: 20px 0; }
  .hrja-modal-meta { background: #f5f6ff; border: 1px solid #e0e7ff; border-radius: 10px; padding: 12px 16px; margin-bottom: 14px; font-size: 13px; color: #6b7280; display: flex; flex-direction: column; gap: 4px; }
  .hrja-modal-meta span strong { color: #1e1b3a; font-weight: 600; }
  .hrja-modal-list { margin: 6px 0 0; padding-left: 18px; line-height: 1.8; }
  .hrja-modal-list li { margin-bottom: 2px; }
  .hrja-modal-error { background: #fef2f2; border: 1px solid #fecaca; border-radius: 10px; padding: 14px 16px; font-size: 13px; color: #b91c1c; margin-bottom: 14px; }
  ...
  @keyframes hrja-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
  .hrja-processing { animation: hrja-pulse 2s ease-in-out infinite; }
...
function HRJobApplications() {
  ...
  const applyFilter = (count) => {
    ...
    if (n <= 0) { setApplications([]); setIsFiltered(true); return; }
    setApplications(allApplications.slice(0, n));
    setIsFiltered(true);
  };
  ...
  // ✅ FIXED: matches normalized backend response + handles error field from failed retries
  const getHFStatus = (app) => {
    // null = background thread hasn't finished yet (still processing)
    if (app.gradio_match_score === null || app.gradio_match_score === undefined) {
      return { type: 'processing', message: 'Processing…' };
    }
    // Backend sets error:true in gradio_analysis when all retries fail
    if (app.gradio_analysis?.error === true) {
      return { type: 'failed', message: 'Failed' };
    }
    // Has analysis data = done (score can be 0 legitimately)
    if (app.gradio_analysis && Object.keys(app.gradio_analysis).length > 0) {
      return { type: 'success', score: app.gradio_match_score };
    }
    // Empty object {} = still processing
    return { type: 'processing', message: 'Processing…' };
  };

  // ✅ FIXED: reads normalized fields (score not Total_score, matching_analysis as string with \n bullets)
  const getAnalysisData = (app) => {
    const hfStatus = getHFStatus(app);
    const analysis = app.gradio_analysis || {};

    if (hfStatus.type === 'success') {
      return {
        // matching_analysis comes as "\n• point1\n• point2" string from backend normalize
        matching_analysis: analysis.matching_analysis || "No analysis provided",
        description: analysis.description || "No description available",
        // Backend normalized 'Total_score' → 'score', fallback to gradio_match_score
        score: analysis.score ?? app.gradio_match_score ?? 0,
    ...
    if (hfStatus.type === 'failed') {
      return {
        matching_analysis: analysis.matching_analysis || "Analysis failed after multiple attempts",
        description: analysis.description || "The HuggingFace Gradio space was unavailable or timed out after all retries.",
        score: 0,
        recommendation: analysis.recommendation || "Please try re-submitting or contact support if this persists.",
        name: "", email: "", phone: "",
        status: 'error'
      };
    }
    ...
    // processing
    return {
      matching_analysis: "Analysis is being processed in the background.",
      description: "The HuggingFace model is currently analyzing this resume.",
      score: 0,
      recommendation: "This page auto-refreshes every 30 seconds.",
      name: "", email: "", phone: "",
      status: 'processing'
    };
  };
  ...
  return (
    <div className="hrja-wrap">
      ...
      <div className="hrja-main">
        ...
        {allApplications.length === 0 ? (
          ...
          <>
            <div className="hrja-table-head">
              <div className="hrja-col-center">Rank</div>
              <div>Applicant</div>
              <div>Email</div>
              <div className="hrja-col-center">Final Score</div>
              <div className="hrja-col-center">Groq</div>
              <div className="hrja-col-center">BERT</div>
              <div className="hrja-col-center">Custom ML</div>
              <div className="hrja-col-center">HF Model</div>
              <div className="hrja-col-center">CGPA</div>
              <div className="hrja-col-center">Actions</div>
            </div>
              ...
              {applications.map((app, idx) => {
                ...
                return (
                  <div key={app.id ?? idx} className={`hrja-row${idx % 2 !== 0 ? " hrja-row--even" : ""}`}>
                    <div className="hrja-col-center" style={{ flexDirection: "column" }}>
                    ...
                    <div className="hrja-col-center">
                      <span className={groqRank ? "hrja-score hrja-score--groq" : "hrja-score-null"}>{groqRank ? `${groqRank.toFixed(1)}%` : "—"}</span>
                    </div>
                    <div className="hrja-col-center">
                      <span className={bertScore ? "hrja-score hrja-score--bert" : "hrja-score-null"}>{bertScore ? `${bertScore.toFixed(1)}%` : "—"}</span>
                    </div>
                    <div className="hrja-col-center">
                      <span className={customML ? "hrja-score hrja-score--custom" : "hrja-score-null"}>{customML ? `${customML.toFixed(1)}%` : "—"}</span>
                    </div>
                    ...
                    <div className="hrja-col-center">
                      <button
                        className={
                          hfStatus.type === 'success' ? "hrja-score hrja-score--hf" :
                          hfStatus.type === 'failed' ? "hrja-score--failed" :
                          "hrja-score--processing hrja-processing"
                        }
                        style={{ background: "none", border: "none", cursor: "pointer", textDecoration: "underline", font: "inherit", fontWeight: hfStatus.type === 'success' ? 700 : 500 }}
                        onClick={() => { setSelectedAnalysis(getAnalysisData(app)); setIsModalOpen(true); }}
                      >
                        {hfStatus.type === 'success'
                          ? `${typeof hfStatus.score === 'number' ? hfStatus.score.toFixed(1) : hfStatus.score}%`
                          : hfStatus.message}
                      </button>
                    </div>

                    <div className="hrja-col-center">
                      <span className="hrja-cgpa"><AcademicCapIcon />{cgpa}</span>
                    </div>
        ...
        {/* ✅ FIXED MODAL — handles all 3 states, renders list or string, shows extra fields */}
        {isModalOpen && selectedAnalysis && (
          <div className="hrja-modal-overlay" onClick={() => setIsModalOpen(false)}>
            <div className="hrja-modal" onClick={e => e.stopPropagation()}>
              ...
              <h3 className="hrja-modal-title">
                {selectedAnalysis.status === 'success' ? '🎯 HuggingFace Model Analysis' :
                 selectedAnalysis.status === 'error' ? '⚠️ Analysis Failed' :
                 '⏳ Analysis In Progress'}
              </h3>

              {/* Score — only shown on success */}
              {selectedAnalysis.status === 'success' && (
                <div className="hrja-modal-score">
              ...
              {/* Error banner — only shown on failed */}
              {selectedAnalysis.status === 'error' && (
                <div className="hrja-modal-error">
                  ❌ The HuggingFace Gradio space failed after all retry attempts. This usually means the free space was sleeping or overloaded.
                </div>
              )}

              {/* Extra applicant info from API (name, email, phone) — only on success */}
              {selectedAnalysis.status === 'success' && (selectedAnalysis.name || selectedAnalysis.email || selectedAnalysis.phone) && (
                <div className="hrja-modal-meta">
                  {selectedAnalysis.name && <span><strong>Candidate:</strong> {selectedAnalysis.name}</span>}
                  {selectedAnalysis.email && <span><strong>Email:</strong> {selectedAnalysis.email}</span>}
                  {selectedAnalysis.phone && <span><strong>Phone:</strong> {selectedAnalysis.phone}</span>}
                </div>
              )}

              {/* ✅ matching_analysis — renders as bullet list if \n• format, else plain text */}
              <div className="hrja-modal-row">
                <strong>Matching Analysis:</strong>
                {Array.isArray(selectedAnalysis.matching_analysis) ? (
                  <ul className="hrja-modal-list">
                    {selectedAnalysis.matching_analysis.map((point, i) => <li key={i}>{point}</li>)}
                  </ul>
                ) : selectedAnalysis.matching_analysis?.includes('\n•') ? (
                  <ul className="hrja-modal-list">
                    {selectedAnalysis.matching_analysis.split('\n').filter(l => l.trim()).map((line, i) => (
                      <li key={i}>{line.replace(/^•\s*/, '')}</li>
                    ))}
                  </ul>
                ) : (
                  <span style={{ whiteSpace: "pre-line" }}>{selectedAnalysis.matching_analysis}</span>
                )}
              </div>

              <div className="hrja-modal-row">
                <strong>Description:</strong>
                <span style={{ whiteSpace: "pre-line" }}>{selectedAnalysis.description}</span>
              </div>

              <div className="hrja-modal-row">
                <strong>Score:</strong>
                {typeof selectedAnalysis.score === 'number' ? selectedAnalysis.score.toFixed(1) : selectedAnalysis.score}%
              </div>

              <div className="hrja-modal-row">
                <strong>Recommendation:</strong>
                <span style={{ whiteSpace: "pre-line" }}>{selectedAnalysis.recommendation}</span>
              </div>

              {selectedAnalysis.status === 'processing' && (
                <div style={{ marginTop: 16, padding: 12, background: "#fef9c3", borderRadius: 8, fontSize: 12, color: "#854d0e", textAlign: "center" }}>
